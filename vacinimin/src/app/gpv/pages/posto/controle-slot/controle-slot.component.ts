...
import { SlotHorarioFormErrors } from 'src/app/core/_models/slot/slot-horario-form-errors';
...
export class ControleSlotComponent implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute,  private fb: FormBuilder, private _ss: SlotService, private _ps: PostoService, private _auth_service: AuthenticationService) { 
    this.route.queryParams.subscribe(params => {
      this.postoVacinacaoId = this._auth_service.getLocalVariable(LocalVariable.GPVStationId)
  ...
  createForms() {
    ...
    this.slotDataForm = this.fb.group({
      ...
      priority: new FormControl({value: '', disabled: false}, Validators.compose([])),
    });
  }
  ...
  goToSlotContent(slot) {
    this.router.navigate(['gpv', 'slot'], { queryParams: { idPosto: this.postoVacinacaoId, idCalendar: this.calendarId, idSlot: slot.id }});   
  }
